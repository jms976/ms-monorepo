...
export const dialogVariants = tv({
  slots: {
    ...
    header: 'py-4 px-6 bg-juiPrimary text-juiText-primary flex flex-col gap-2 text-center sm:text-left h-[55px]',
    ...
    closeButton:
      'text-white ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-juiText-secondary absolute top-4 right-6 my-0.5 rounded-xs opacity-80 transition-opacity hover:opacity-100 focus:ring-1 focus:ring-offset-1 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*="size-"])]:size-5',
  },
  variants: {
    contentSize: {
      small: {
